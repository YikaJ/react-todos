...
var start = function start(injector, config, launcher, globalEmitter, preprocess, fileList, webServer, capturedBrowsers, socketServer, executor, done) {
  ...
  var afterPreprocess = function afterPreprocess() {
    ...
    webServer.listen(config.port, function () {
      log.info('Karma v%s server started at http://%s:%s%s', constant.VERSION, config.hostname, config.port, config.urlRoot);
...
start.$inject = ['injector', 'config', 'launcher', 'emitter', 'preprocess', 'fileList', 'webServer', 'capturedBrowsers', 'socketServer', 'executor', 'done'];

var createSocketIoServer = function createSocketIoServer(webServer, executor, config) {
...
exports.start = function (cliOptions, done) {
  ...
  logger.setup(cliOptions.logLevel || constant.LOG_INFO, helper.isDefined(cliOptions.colors) ? cliOptions.colors : true, [constant.CONSOLE_APPENDER]);
