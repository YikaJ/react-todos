...
var createWinGlob = function createWinGlob(realGlob) {
...
var File = function File(path, mtime) {
...
var byPath = function byPath(a, b) {
  if (a.path > b.path) {
    return 1;
  }
  if (a.path < b.path) {
...
var List = function List(patterns, excludes, emitter, preprocess, batchInterval) {
  ...
  var addError = function addError(path) {
    if (errors.indexOf(path) === -1) {
      errors.push(path);
    }
  };

  var removeError = function removeError(path) {
  ...
  var resolveFiles = function resolveFiles(buckets) {
    var uniqueMap = {};
    var files = {
      served: [],
      included: []
  ...
  var resolveDeferred = function resolveDeferred(files) {
    clearPendingTimeout();
  ...
  var fireEventAndDefer = function fireEventAndDefer() {
    clearPendingTimeout();
  ...
  var clearPendingTimeout = function clearPendingTimeout() {
    if (pendingTimeout) {
    ...
    patterns.forEach(function (patternObject, i) {
      ...
      glob(pattern, GLOB_OPTS, function (err, resolvedFiles) {
        ...
        resolvedFiles.forEach(function (path) {
          var matchExclude = function matchExclude(excludePattern) {
...
//# sourceMappingURL=file_list-compiled.js.map